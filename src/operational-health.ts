...
import type { ContainerStatus, DashboardPayload, MonitorDetailPage, TimeRange } from './types';
...
export function operationalFindings(data: DashboardPayload): OperationalFinding[] {
  ...
  if (networkFaultRate >= NETWORK_FAULT_RATE_THRESHOLDS.caution) {
    ...
    findings.push(finding(
      'network-quality',
      networkFaultRate >= NETWORK_FAULT_RATE_THRESHOLDS.danger ? 'danger' : 'caution',
      snapshotScope,
      [
        observedNetworkFaults.map(([ko, , value]) => `${ko} ${formatRate(value)}`).join(' · '),
        observedNetworkFaults.map(([, en, value]) => `${en} ${formatRate(value)}`).join(' · '),
      ],
      null,
      snapshotObservedAt,
    ));
  ...
  if (storageErrorCount || readOnlyMounts.length) {
    ...
    findings.push(finding(
      'storage-integrity',
      'danger',
      readOnlyMounts.length ? snapshotScope : bootScope,
      [`파일시스템 ${numericCount(kernel.filesystemError.count)} · NVMe reset ${numericCount(kernel.nvmeReset.count)} · I/O ${numericCount(kernel.nvmeIo.count)}${readOnlyKo}`, `filesystem ${numericCount(kernel.filesystemError.count)} · NVMe reset ${numericCount(kernel.nvmeReset.count)} · I/O ${numericCount(kernel.nvmeIo.count)}${readOnlyEn}`],
      storageErrorCount + readOnlyMounts.length,
      readOnlyMounts.length ? snapshotObservedAt : latestTimestamp([kernel.filesystemError.lastEventAt, kernel.nvmeReset.lastEventAt, kernel.nvmeIo.lastEventAt]),
    ));
  }
