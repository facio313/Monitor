...
describe('system emotion engine presentation', () => {
  it('renders a labeled particle surface without operational overlays', () => {
    ...
    expect(markup).toContain('aria-label="현재 시스템 상태의 앰버 입자 유체 파동"');
    expect(markup).toContain('aria-hidden="true"');
    expect(markup).toContain('data-mood="dormant"');
    expect(markup).toContain('data-renderer="amber-fluid-particle-wave"');
  ...
  it('builds a deterministic, dense particle sheet at desktop and compact sizes', () => {
    ...
    expect(desktop.strataPerAxis).toBe(6);
    expect(desktop.strandCount).toBe(model.axes.length * 6);
    expect(desktop.waveParticleCount).toBeGreaterThanOrEqual(6_800);
    expect(desktop.waveParticleCount).toBeLessThanOrEqual(7_500);
    expect(desktop.waveBuckets.flat()).toHaveLength(desktop.waveParticleCount);
    expect(desktop.sprayBuckets.flat()).toHaveLength(desktop.sprayParticleCount);
    expect(desktopAgain).toEqual(desktop);

    expect(compact.compact).toBe(true);
    expect(compact.strataPerAxis).toBe(5);
    expect(compact.strandCount).toBe(model.axes.length * 5);
    expect(compact.waveParticleCount).toBeGreaterThanOrEqual(2_100);
    expect(compact.waveParticleCount).toBeLessThanOrEqual(3_300);
