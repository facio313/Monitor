...
function networkRisk(data: DashboardPayload): number | null {
  ...
  const faultObserved = faultValues.some(finite);
  const faultRate = faultValues.reduce((total: number, value) => total + (finite(value) ? value : 0), 0);
  const faultRisk = faultObserved ? ramp(faultRate, 0, NETWORK_FAULT_RATE_THRESHOLDS.danger) : null;
...
export function deriveSystemEmotion({
  ...
  const activeIncidents = data.incidents.filter((incident) => incident.phase === 'active').length;
