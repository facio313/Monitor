...
class ContainerExporterTests(unittest.TestCase):
    ...
    def event_result(self):
        collection = container_exporter.collector.unavailable_docker_event_collection()
    ...
    def test_writes_only_reduced_snapshot_and_private_cpu_state(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(container_exporter.os, "geteuid", return_value=1001), \
                 mock.patch.object(
                     container_exporter.collector, "collect_docker_events",
                     return_value=self.event_result(),
                 ), \
            ...
            self.assertEqual(public["dockerEventCollection"], self.event_result()[0])
            self.assertEqual(public["dockerEvents"], [])
            self.assertEqual(set(private), {"generatedAt", "containers", "dockerEvents"})
    ...
    def test_unavailable_source_exports_last_known_without_replacing_cpu_state(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            state.write_text(json.dumps({"containers": {"cks:" + "a" * 64: {
                "cpuTotal": 1, "systemTotal": 2, "onlineCpus": 1, "restartCount": 3,
            }}}) + "\n", encoding="utf-8")
            calls = 0
            ...
            with mock.patch.object(container_exporter.os, "geteuid", return_value=1001), \
                 mock.patch.object(
                     container_exporter.collector,
                     "load_container_snapshot_document",
                     return_value=(
                         [previous_container],
            ...
            self.assertEqual(private["containers"], {"cks:" + "a" * 64: {
                "cpuTotal": 1, "systemTotal": 2, "onlineCpus": 1, "restartCount": 3,
            }})
