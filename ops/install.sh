...
synthetic_doc_target=/usr/local/share/doc/monitor-collector/synthetic-probes.md
synthetic_example_target=/usr/local/share/doc/monitor-collector/synthetic-probes.example.json
...
synthetic_service_target=/etc/systemd/system/monitor-synthetic-probe.service
synthetic_timer_target=/etc/systemd/system/monitor-synthetic-probe.timer
...
for source in \
    ...
    "$script_dir/../docs/synthetic-probes.md" \
    "$script_dir/synthetic-probes.example.json" \
    ...
    "$script_dir/systemd/monitor-synthetic-probe.service" \
    "$script_dir/systemd/monitor-synthetic-probe.timer" \
...
had_synthetic_doc=false
had_synthetic_example=false
...
had_synthetic_service=false
had_synthetic_timer=false
...
synthetic_output_directory_uid=
synthetic_output_directory_gid=
synthetic_output_directory_mode=
committed=false
...
was_synthetic_timer_enabled=false
was_synthetic_timer_active=false
...
finish() {
    ...
    if [ "$transaction_started" = true ] && [ "$committed" != true ]; then
        systemctl stop monitor-synthetic-probe.timer monitor-synthetic-probe.service monitor-alert-delivery.timer monitor-alert-delivery.service monitor-collector.timer monitor-collector.service monitor-container-exporter.service >/dev/null 2>&1 || true
        ...
        restore_file "$backup_dir/synthetic-probes.md" "$synthetic_doc_target" "$had_synthetic_doc" || rollback_failed=true
        restore_file "$backup_dir/synthetic-probes.example.json" "$synthetic_example_target" "$had_synthetic_example" || rollback_failed=true
        ...
        restore_file "$backup_dir/monitor-synthetic-probe.service" "$synthetic_service_target" "$had_synthetic_service" || rollback_failed=true
        restore_file "$backup_dir/monitor-synthetic-probe.timer" "$synthetic_timer_target" "$had_synthetic_timer" || rollback_failed=true
        ...
        restore_directory /var/lib/monitor-synthetic "$had_synthetic_output_directory" "$created_synthetic_output_directory" "$synthetic_output_directory_uid" "$synthetic_output_directory_gid" "$synthetic_output_directory_mode" || rollback_failed=true
        ...
        if [ "$was_synthetic_timer_enabled" = true ]; then
            systemctl enable monitor-synthetic-probe.timer >/dev/null 2>&1 || rollback_failed=true
        else
            systemctl disable monitor-synthetic-probe.timer >/dev/null 2>&1 || true
        fi
        if [ "$was_synthetic_timer_active" = true ]; then
            systemctl start monitor-synthetic-probe.timer >/dev/null 2>&1 || rollback_failed=true
        fi
    fi
    ...
    rm -f \
        ...
        "$backup_dir/synthetic-probes.md" \
        "$backup_dir/synthetic-probes.example.json" \
        ...
        "$backup_dir/monitor-synthetic-probe.service" \
        "$backup_dir/monitor-synthetic-probe.timer"
...
for target in \
    ...
    "$synthetic_doc_target" \
    "$synthetic_example_target" \
    ...
    "$synthetic_service_target" \
    "$synthetic_timer_target" \
...
if systemctl is-enabled --quiet monitor-synthetic-probe.timer 2>/dev/null; then
    was_synthetic_timer_enabled=true
fi
if systemctl is-active --quiet monitor-synthetic-probe.timer 2>/dev/null; then
    was_synthetic_timer_active=true
fi
...
if [ -e "$synthetic_doc_target" ]; then cp -p "$synthetic_doc_target" "$backup_dir/synthetic-probes.md"; had_synthetic_doc=true; fi
if [ -e "$synthetic_example_target" ]; then cp -p "$synthetic_example_target" "$backup_dir/synthetic-probes.example.json"; had_synthetic_example=true; fi
...
if [ -e "$synthetic_service_target" ]; then cp -p "$synthetic_service_target" "$backup_dir/monitor-synthetic-probe.service"; had_synthetic_service=true; fi
if [ -e "$synthetic_timer_target" ]; then cp -p "$synthetic_timer_target" "$backup_dir/monitor-synthetic-probe.timer"; had_synthetic_timer=true; fi
...
if [ -d /var/lib/monitor-synthetic ]; then
    had_synthetic_output_directory=true
    capture_directory_metadata /var/lib/monitor-synthetic
    synthetic_output_directory_uid=$captured_directory_uid
    synthetic_output_directory_gid=$captured_directory_gid
    synthetic_output_directory_mode=$captured_directory_mode
fi

transaction_started=true
systemctl stop monitor-synthetic-probe.timer monitor-synthetic-probe.service >/dev/null 2>&1 || true
...
for unit in monitor-synthetic-probe.timer monitor-synthetic-probe.service monitor-alert-delivery.timer monitor-alert-delivery.service monitor-collector.timer monitor-collector.service monitor-container-exporter.service
do
...
if [ "$had_synthetic_output_directory" != true ]; then
    mkdir -- /var/lib/monitor-synthetic
    created_synthetic_output_directory=true
fi
install -d -o cks -g cks -m 0750 /var/lib/monitor-synthetic
...
committed=true
echo "Installed. Alert delivery and synthetic probes retained their prior timer states."
echo "Synthetic probes remain opt-in; provision /etc/monitor-synthetic-probe/probes.json before first enablement."
echo "Inspect with: systemctl status monitor-collector.timer monitor-collector.service monitor-alert-delivery.timer monitor-synthetic-probe.timer"
